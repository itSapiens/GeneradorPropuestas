...
export async function signContractUseCase(
  ...
  const uploadedContract = await deps.services.documents.uploadClientDocument({
    ...
    documentSetId: ctx.study?.source_file?.document_set_id ?? ctx.study.id,
    dni: ctx.client.dni,
  ...
  const updatedContract = await deps.repositories.contracts.update(contract.id, {
    ...
    metadata: {
      ...
      signed_contract: {
        ...
        file_name: uploadedContract.fileName,
        folder_path: uploadedContract.folderPath,
        mime_type: uploadedContract.mimeType,
        original_name: params.signedContractFile.originalname ?? null,
        path: uploadedContract.path,
