...
import { ValidationBillData } from "@/src/entities/proposal/domain/proposal.types";
...
export function getInvoiceTotalAmountFromExtraction(
  extraction: ExtractedBillData | null,
): number | undefined {
  const invoiceData = extraction?.invoice_data as
    | Record<string, unknown>
...
export function mapExtractedToBillData(
  ...
  return {
    ...
    billedDays: normalizeAndRoundUp(data.invoice_data.billedDays, 0),
    invoiceTotalAmountEur: normalizeAndRoundUp(
      data.invoice_data.invoiceTotalAmountEur,
      2,
    ),
