...
export interface CalculationResult {
  ...
  estimatedAnnualInvoiceCost: number;
...
export const calculateEnergyStudy = (
  ...
  const validInvoiceTotalAmount =
    typeof input.invoiceTotalAmountEur === "number" &&
    Number.isFinite(input.invoiceTotalAmountEur) &&
    input.invoiceTotalAmountEur > 0;
  const validBilledDays =
    typeof input.billedDays === "number" &&
    Number.isFinite(input.billedDays) &&
    input.billedDays > 0;
  const estimatedAnnualInvoiceCost = round(
    validInvoiceTotalAmount && validBilledDays
      ? (input.invoiceTotalAmountEur! / input.billedDays!) * 365
