...
import {
  ...
  renderSignedContractPdfUseCase,
...
export function createContractController(deps: ServerDependencies) {
  return {
    ...
    async renderSignedPdf(req: Request, res: Response) {
      try {
        const pdfBuffer = await renderSignedContractPdfUseCase(deps, {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="contrato-firmado.pdf"',
        );
