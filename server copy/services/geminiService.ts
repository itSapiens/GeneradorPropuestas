...
const SYSTEM_INSTRUCTION = [
  ...
  "- invoice_data.invoiceTotalAmountEur: total final de la factura con impuestos, si aparece.",
  "  Busca líneas como 'TOTAL IMPORTE FACTURA 133,00 €' o 'TOTAL 133,00 €'.",
  ...
  properties: {
    ...
    invoice_data: {
      ...
      properties: {
        ...
        invoiceTotalAmountEur: { type: Type.NUMBER, nullable: true },
...
function extractInvoiceTotalAmount(text: string): number | null {
  return (
    ...
    parseSpanishNumber(
      extractRegexValue(text, /TOTAL\s+([\d.,]+)\s*€/i),
    )
  );
}
...
invoice_data: {
  ...
  invoiceTotalAmountEur: safeNumber(data?.invoice_data?.invoiceTotalAmountEur),
  periods: {
  ...
  const merged: ExtractedBillData = {
    ...
    invoice_data: {
      ...
      invoiceVariableEnergyAmountEur:
        ...
        null,
      invoiceTotalAmountEur:
        localData.invoice_data?.invoiceTotalAmountEur ??
        aiData.invoice_data.invoiceTotalAmountEur ??
        null,
