...
export async function getInstallationCapacityState(params: {
  ...
  const { data: installation, error: installationError } = await supabase
    .from("installations")
    .select(
      "id, nombre_instalacion, direccion, lat, lng, potencia_instalada_kwp, contractable_kwp_total, contractable_kwp_reserved, contractable_kwp_confirmed, active, calculo_estudios, potencia_fija_kwp, pago, cantidad_precio_fijo, reserva, reserva_fija_eur, iban_aportaciones",
    )
...
export function buildInstallationSnapshot(params: {
  ...
  return {
    ...
    installationData: {
      ...
      pago: (params.installation as any).pago ?? null,
      cantidad_precio_fijo:
        (params.installation as any).cantidad_precio_fijo ?? null,
    },
