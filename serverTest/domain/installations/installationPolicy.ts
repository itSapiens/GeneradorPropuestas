...
export function resolveInstallationBankBeneficiary(
  installation: any,
  fallbackBeneficiary: string,
): string {
  return (
    pickFirstString(
      installation?.empresa?.nombre,
      installation?.empresas?.nombre,
      fallbackBeneficiary,
    ) ?? fallbackBeneficiary
  );
}

export function resolveInstallationContactEmail(
  installation: any,
  fallbackEmail: string,
): string {
  return (
    pickFirstString(
      installation?.empresa?.email,
      installation?.empresas?.email,
      fallbackEmail,
    ) ?? fallbackEmail
  );
}
...
export function buildInstallationSnapshot(params: {
  ...
  return {
    ...
    installationData: {
      ...
      pago: params.installation.pago ?? null,
      cantidad_precio_fijo: params.installation.cantidad_precio_fijo ?? null,
